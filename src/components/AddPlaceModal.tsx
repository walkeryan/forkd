  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-lg bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-stone-950/20 max-h-[88vh] flex flex-col animate-rise">
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-stone-200/60">
          <h2 className="text-lg font-bold text-stone-900">
            {step === 'meal' ? 'Log a Meal' : 'Add a Place'}
          </h2>
          <button onClick={step === 'meal' ? finishMealStep : onClose} className="text-stone-400 p-1" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {step === 'meal' && addedPlace ? (
          <div className="overflow-y-auto px-5 py-4 space-y-4">
            <div>
              <h3 className="text-base font-bold tracking-tight text-stone-900">You&apos;re at <span className="text-orange-600">{addedPlace.name}</span>! What did you have?</h3>
              <p className="text-sm text-stone-400 mt-0.5">Log your dish now, or skip and add it later.</p>
            </div>
            <MealForm
            ...
            <button
              ...
              className="w-full text-center text-sm text-stone-500 py-1 active:text-stone-700"
            >
              Skip for now
            </button>
          </div>
        ...
        <div className="overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-1 bg-stone-100 rounded-xl p-1">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 rounded-lg py-2 text-sm font-medium transition ${mode === 'visited' ? 'bg-white text-orange-600 shadow-sm' : 'text-stone-500'}`}
            >
              <Check className="w-4 h-4" /> Visited
            </button>
            <button
              onClick={() => setMode('wishlist')}
              className={`flex items-center justify-center gap-1.5 rounded-lg py-2 text-sm font-medium transition ${mode === 'wishlist' ? 'bg-white text-teal-600 shadow-sm' : 'text-stone-500'}`}
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white border border-stone-200 rounded-2xl pl-9 pr-9 py-3 text-sm shadow-sm placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-orange-400/70 focus:border-orange-300 transition-shadow"
            />
            {searching && (
              ...
              {displayResults.map(({ place: p, distance }) => {
                ...
                return (
                  <button
                    ...
                    className="card w-full text-left flex items-start gap-3 p-3 active:scale-[0.99] active:bg-orange-50/40 transition-all duration-150 disabled:opacity-60"
                  >
                    <ResultThumb place={p} />
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-stone-900 truncate">{p.name}</p>
                      {meta && <p className="text-xs text-stone-500 mt-0.5">{meta}</p>}
                      {p.address && <p className="text-xs text-stone-400 truncate">{p.address}</p>}
                    </div>
          ...
          <div className="border-t border-stone-200/60 pt-3">
            {!manualOpen ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-sm text-stone-500 py-2"
              >
              ...
              <form onSubmit={submitManual} className="space-y-2">
                <input
                  ...
                  className="w-full bg-white border border-stone-200 rounded-xl px-4 py-3 text-sm placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-orange-400/70 focus:border-orange-300 transition-shadow"
                />
                <input
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-b from-orange-500 to-orange-600 text-white rounded-xl py-3 font-semibold text-sm shadow-sm shadow-orange-500/20 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
